...
def test_connection(params: dict) -> dict:
    ...
    if db_type == "sqlserver":
        ...
        try:
            ...
            conn.close()
            return {"success": True, "message": f"Connected to SQL Server {host}:{port}", "latency_ms": int((time.perf_counter() - t0) * 1000)}
    ...
    try:
        ...
        conn.close()
        return {"success": True, "message": f"Connected to PostgreSQL {host}:{port}", "latency_ms": int((time.perf_counter() - t0) * 1000)}
    except Exception as exc:
        return {"success": False, "message": str(exc)}
